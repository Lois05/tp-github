...
@section('content')
    <div class="container py-4">
        ...
        <form method="GET" action="{{ route('admin.biens.index') }}" class="row g-2 align-items-center mb-4">
            <div class="col-md-4">
                <input type="text" name="search" class="form-control" placeholder="Rechercher par nom ou catégorie"
            ...
            <table class="table table-striped align-middle">
                <thead class="table-light">
                    <tr>
                        ...
                        <th>Nom</th>
                        <th>Catégorie</th>
                        <th>Propriétaire</th> {{-- ✅ Nouvelle colonne --}}
                        <th>Statut</th>
                        <th>Créé le</th>
                        <th>Actions</th>
                    </tr>
                ...
                <tbody>
                    @forelse($biens as $bien)
                        <tr>
                            ...
                            <td>{{ $bien->proprietaire->nom ?? 'Aucun propriétaire' }}</td>
                            ...
                            <td>
                                <div class="btn-group" role="group" aria-label="Actions bien">
                                    ...
                                    <form action="{{ route('admin.biens.destroy', $bien) }}" method="POST"
                                        class="d-inline me-1" onsubmit="return confirm('Supprimer ce bien ?')">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <form action="{{ route('admin.biens.toggleEtat', $bien) }}" method="POST"
                                        class="d-inline" onsubmit="return confirm('Changer le statut du bien ?')">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
