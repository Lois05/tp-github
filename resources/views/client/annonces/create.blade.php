        ...
        <form method="POST" action="{{ route('client.annonce.store') }}" enctype="multipart/form-data" novalidate>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('titre') is-invalid @enderror" id="titre"
                    name="titre" value="{{ old('titre') }}" required placeholder="Entrez le titre de votre annonce">
                @error('titre')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('localisation') is-invalid @enderror" id="localisation"
                    name="localisation" value="{{ old('localisation') }}" required
            ...
            <div class="mb-3">
                ...
                <select name="etat" id="etat" class="form-select @error('etat') is-invalid @enderror">
                    ...
                    <option value="en maintenance" {{ old('etat') == 'en maintenance' ? 'selected' : '' }}>En maintenance
                    </option>
                    <option value="indisponible" {{ old('etat') == 'indisponible' ? 'selected' : '' }}>Indisponible
                    </option>
                </select>
                @error('etat')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('prix') is-invalid @enderror" id="prix" name="prix"
                    value="{{ old('prix') }}" min="0" required placeholder="Exemple : 50000">
                @error('prix')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                    rows="5" required placeholder="Décrivez votre bien en quelques mots...">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
